...
const SHOP_TOWERS = {
  ...
  hypefarmer: {
    ...
    icon:'🎉', isFarmer:true,
    desc:'Renda menor que o Fazendeiro comum, mas a cada onda tem chance de jogar uma caixa-surpresa: ouro bônus, uma gema grátis, ou um Hype raro que acelera TODAS as torres do mapa por alguns segundos.',
    upgrades:[
...
const ADMIN_TOWERS = {
  gojo: {
    ...
    upgrades:[
      { label:'Infinidade',      dmg:80,  range:110, rate:1.0, splash:80,  upgCost:0,    technique:'infinity'  },
      { label:'Vazio Azul',      dmg:180, range:200, rate:0.7, splash:50,  upgCost:600,  technique:'blue'      },
      { label:'Expansão Máxima', dmg:600, range:260, rate:0.35,splash:200, upgCost:1200, technique:'purple'    },
    ]
  },
  ...
  umbra: {
    ...
    desc:'Entidade do vazio que se alimenta de luz. Cada disparo CONGELA e CORRÓI a defesa dos inimigos ao mesmo tempo — controle de grupo em estado puro.',
    isFreeze: true, isVuln: true,
    upgrades:[
      { label:'Sombra Nascente',  dmg:40,  range:130, rate:0.9, splash:70,  upgCost:0,
        freezeDuration:2.0, freezeStrength:0.40, vulnMult:1.30, vulnDuration:3.5 },
      { label:'Penumbra Devoradora', dmg:70, range:150, rate:1.1, splash:90, upgCost:280,
        freezeDuration:2.5, freezeStrength:0.50, vulnMult:1.45, vulnDuration:4.0 },
      { label:'Colapso Umbrio',   dmg:115, range:170, rate:1.3, splash:115, upgCost:550,
        freezeDuration:3.0, freezeStrength:0.62, vulnMult:1.65, vulnDuration:4.5 },
    ]
  },
  ...
  nemesis: {
    ...
    desc:'A estrela que persegue. Disparos de energia cósmica saltam entre vários alvos em cadeia e deixam um rastro de decomposição estelar (veneno) em cada um que atingem.',
    isChain: true, isPoison: true,
    upgrades:[
      { label:'Caçadora',            dmg:45,  range:140, rate:0.85, splash:0, upgCost:0,
        chainRange:60, chainCount:3, poisonDps:16, poisonDuration:3.0 },
      { label:'Perseguição Eterna',  dmg:78,  range:160, rate:1.05, splash:0, upgCost:320,
        chainRange:72, chainCount:4, poisonDps:26, poisonDuration:3.5 },
      { label:'Veredito Final',      dmg:130, range:185, rate:1.25, splash:0, upgCost:600,
        chainRange:85, chainCount:5, poisonDps:42, poisonDuration:4.0 },
    ]
  },
};
...
function addUnlockedTowerToPanel(key, def) {
  ...
  btn.innerHTML = `
    <div class="tbtn-icon" style="background:linear-gradient(135deg,${def.color},${def.glow})">${def.icon}</div>
...
function addAdminTowerToPanel(key, def) {
  ...
  btn.setAttribute('data-type', key);
  btn.innerHTML = `
    <div class="tbtn-icon" style="background:linear-gradient(135deg, ${def.color}, ${def.glow}); box-shadow:0 0 10px ${def.glow};">${def.icon}</div>
...
function fireTowers(dt) {
  ...
  state.towers.forEach(tower => {
    ...
    if (tower.type === 'perfurador') {
      ...
      hits.slice(0,maxHits).forEach(h=>damageEnemy(h.e,stats.dmg));
